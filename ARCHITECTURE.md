...
├── vite.config.ts               ← thin wrapper over @lovable.dev preset
├── tsconfig.json                ← strict TS, @/* alias
├── components.json              ← shadcn/ui config
├── eslint.config.js
├── .prettierrc, .prettierignore, bunfig.toml
└── src/
    ...
    └── lib/
        ├── utils.ts             ← cn() helper
        ├── error-capture.ts     ← client error → Lovable relay
        ├── error-page.ts        ← full-page error fallback
...
2. **Persist state.** Enable Lovable Cloud (Supabase under the hood). Model tables mirror Section 4 types (`tasks`, `routines`, `appointments`, `helpers`, `admins`, `schedules`, `pantry_items`, `grocery_items`, `vales`, `ledger_entries`, `invites`, `invite_flags`, `quick_utos`, `notes`). Follow the project's user-roles pattern (separate `user_roles` table + `has_role` SECURITY DEFINER; never store roles on profiles). Always pair `CREATE TABLE public.*` with explicit `GRANT`s and RLS policies.
3. **Server functions.** Move mutations to `createServerFn` in `src/lib/*.functions.ts`, protected by the `requireSupabaseAuth` middleware. Public webhooks (e.g. SMS invite) go under `src/routes/api/public/*` with signature verification.
4. **Real-time.** Use Supabase realtime channels for `tasks`, `utos`, and `ledger` so manager and helper devices stay in sync without polling.
5. **Auth.** Managed Supabase auth. Invite codes redeem into a real account owned by the helper (matching the "record stays yours" copy).
6. **Media.** Move Done photos and voice notes to Supabase Storage; keep `photo` fields as URLs.
...
- Build: `bun run build` (Nitro edge output). Dev build: `bun run build:dev`.
- Typecheck runs automatically in the Lovable harness (`tsgo --noEmit`).
...
## 9. About the AI that authored this codebase

This project was built inside **Lovable**, an AI coding agent that edits the repository, runs commands, and previews the app live. Lovable is powered by a rotating mix of frontier LLMs (currently Anthropic's Claude family among others); the specific model is not exposed to the app and can change over time as new models ship. The models that Lovable-built apps themselves call (via the Lovable AI Gateway) are separate from the model authoring the code and are configurable per project.
