...
1.  Submitting *"laundry every Tuesday"* returns classification `ROUTINE` with station `Laundry`.
2.  Submitting a helper-written scratch note returns classification `PRIVATE_NOTE`.
3.  Sending *"please get more water"* while the targeted helper is off-shift returns `boundaryWarn: true` to trigger the confirmation friction dialog.
