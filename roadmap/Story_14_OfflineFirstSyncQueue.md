...
2.  **Intercept Network Calls:** Refactor the API mutation triggers in `<HelperView />`. If `navigator.onLine === false`, divert the payload to the local offline queue and display a comforting, warm Taglish toast: _"Naka-save offline! Aayusin natin pag may internet na ulit."_
