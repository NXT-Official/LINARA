...
5.  **Write Initial Smoke Tests:**
    - _Unit Test:_ Create `src/lib/time.test.ts` to test time helper utilities (e.g. validating shift hour computations).
    - _E2E Test:_ Create `tests/claims-smoke.spec.ts` to boot the headless browser, visit `/`, and verify that the invitation claim onboarding terms render without throwing frontend script exceptions.
